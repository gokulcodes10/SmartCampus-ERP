...
/**
 * Persistence access for {@link ResumeAchievement}.
 *
 * <p>Supports reading achievement entries in display order and bulk deletion by resume.
 */
public interface ResumeAchievementRepository extends JpaRepository<ResumeAchievement, Long> {

    /**
     * Finds all achievement entries for a resume, ordered for rendering.
     *
     ...
     * @return list of achievement entries in display order, deterministically
     */
    List<ResumeAchievement> findByResumeIdOrderByDisplayOrderAscIdAsc(Long resumeId);

    /**
     * Bulk deletes all achievement entries for a resume.
     *
     ...
     * single transaction.
     *
     * @param resumeId the resume ID whose achievement entries should be deleted
     */
    @Modifying(flushAutomatically = true, clearAutomatically = true)
    @Query("delete from ResumeAchievement x where x.resume.id = :resumeId")
