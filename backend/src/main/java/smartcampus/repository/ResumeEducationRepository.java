...
/**
 * Persistence access for {@link ResumeEducation}.
 *
 * <p>Supports reading education entries in display order and bulk deletion by resume.
 */
public interface ResumeEducationRepository extends JpaRepository<ResumeEducation, Long> {

    /**
     * Finds all education entries for a resume, ordered for rendering.
     *
     ...
     * @return list of education entries in display order, deterministically
     */
    List<ResumeEducation> findByResumeIdOrderByDisplayOrderAscIdAsc(Long resumeId);

    /**
     * Bulk deletes all education entries for a resume.
     *
     ...
     * single transaction.
     *
     * @param resumeId the resume ID whose education entries should be deleted
     */
    @Modifying(flushAutomatically = true, clearAutomatically = true)
    @Query("delete from ResumeEducation x where x.resume.id = :resumeId")
